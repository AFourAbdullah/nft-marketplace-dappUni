...
describe("NFTMarketplace", function () {
  ...
  describe("Making marketplace items", function () {
    ...
    it("Should track newly created item, transfer NFT from seller to marketplace and emit ItemListed event", async function () {
      ...
      await expect(
        marketplace.connect(addr1).listNfts(nft.address, 1, toWei(price))
      )
    ...
    it("Should fail if price is set to zero", async function () {
      await expect(
        marketplace.connect(addr1).listNfts(nft.address, 1, 0)
      ).to.be.revertedWith("Price of nft must be greater than zero!!");
    });
  });
  describe("Purchasing marketplace items", function () {
    ...
    beforeEach(async function () {
      ...
      await marketplace.connect(addr1).listNfts(nft.address, 1, toWei(price));
    });
    ...
    it("Should fail for invalid item ids, sold items and when not enough ether is paid", async function () {
      ...
      ).to.be.revertedWith("item not foun");
      await expect(
      ...
      await expect(
        marketplace.connect(addr2).purchaseNft(1, { value: toWei(price) })
      ).to.be.revertedWith("not enough eth send");
